...
*/
public class PopularStatRequest extends BaseRequest<PopularStatResponse>{
...
// http://api-metrika.wwdi.ru/stat/stat/geo.json?id=2138128&pretty=1&oauth_token=05dd3dd84ff948fdae2bc4fb91f13e22&date1=%s&date2=dateTo";
    private static final String URL = Webservice.API_BASE_URL+"/stat/content/popular.json";

    public PopularStatRequest(boolean useCache, Listener<PopularStatResponse> listener) {
    ...
    //                put("pretty", "1");
            ...
            }, PopularStatResponse.class, listener);
    }


    @Override
    protected PopularStatResponse getCachedResponse() {
        return new Select().from(PopularStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
    }

    @Override
    protected void removeResponseFromCache() {
        PopularStatResponse response = new Select().from(PopularStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
    ...
    @Override
    protected void beforeResponseDispatching(PopularStatResponse response) {
