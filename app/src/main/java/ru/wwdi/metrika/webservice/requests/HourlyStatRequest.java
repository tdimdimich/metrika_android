...
*/
public class HourlyStatRequest extends BaseRequest<HourlyStatResponse>{
...
// http://api-metrika.wwdi.ru/stat/traffic/summary.json?id=2138128&pretty=1&oauth_token=05dd3dd84ff948fdae2bc4fb91f13e22&date1=%s&date2=dateTo";
    private static final String URL = Webservice.API_BASE_URL+"/stat/traffic/hourly.json";

    public HourlyStatRequest(boolean useCache, Listener<HourlyStatResponse> listener) {
    ...
    //                put("pretty", "1");
            ...
            }, HourlyStatResponse.class, listener);
    }


    @Override
    protected HourlyStatResponse getCachedResponse() {
        return new Select().from(HourlyStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
    }

    @Override
    protected void removeResponseFromCache() {
        HourlyStatResponse response = new Select().from(HourlyStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
        if(response==null) return;
        HoursStat max = response.getMax();
        HoursStat min = response.getMin();
        HoursStat totals = response.getTotals();
        List<HoursStat> itemsA = response.getStats();
        for(HoursStat item: itemsA){
            item.delete();
        }
        response.delete();
    ...
    @Override
    protected void beforeResponseDispatching(HourlyStatResponse response) {
