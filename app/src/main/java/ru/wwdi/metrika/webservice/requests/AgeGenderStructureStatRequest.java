...
*/
public class AgeGenderStructureStatRequest extends BaseRequest<AgeGenderStructureStatResponse>{
...
// http://api-metrika.wwdi.ru/stat/traffic/summary.json?id=2138128&pretty=1&oauth_token=05dd3dd84ff948fdae2bc4fb91f13e22&date1=%s&date2=dateTo";
    private static final String URL = Webservice.API_BASE_URL+"/stat/demography/structure.json";

    public AgeGenderStructureStatRequest(boolean useCache, Listener<AgeGenderStructureStatResponse> listener) {
    ...
    //                put("pretty", "1");
            ...
            }, AgeGenderStructureStatResponse.class, listener);
    }


    @Override
    protected AgeGenderStructureStatResponse getCachedResponse() {
        return new Select().from(AgeGenderStructureStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
    }

    @Override
    protected void removeResponseFromCache() {
        AgeGenderStructureStatResponse response = new Select().from(AgeGenderStructureStatResponse.class).where("startDate='" + mStartDate.getTime() + "'").and("endDate='" + mEndDate.getTime() + "'").executeSingle();
    ...
    @Override
    protected void beforeResponseDispatching(AgeGenderStructureStatResponse response) {
