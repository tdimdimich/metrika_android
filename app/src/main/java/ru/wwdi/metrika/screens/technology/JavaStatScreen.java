 ...
 */
public class JavaStatScreen extends BaseStatisticsScreen {
    ...
    private JavaStatRequest mRequest;
    private JavaStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final JavaStat data = mData.getStats().get(position);
        final JavaStat total = mData.getTotals();
        final JavaStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getJavaStat(!needRefresh(), new BaseRequest.Listener<JavaStatResponse>() {
            @Override
            public void onResponse(JavaStatResponse response) {
