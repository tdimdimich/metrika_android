 ...
 */
public class OSsStatScreen extends BaseStatisticsScreen {
    ...
    private OSStatRequest mRequest;
    private OSStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final OSStat data = mData.getStats().get(position);
        final OSStat total = mData.getTotals();
        final OSStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getOSStat(!needRefresh(), new BaseRequest.Listener<OSStatResponse>() {
            @Override
