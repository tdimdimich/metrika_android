 ...
 */
public class HourlyStatScreen extends BaseStatisticsScreen {
    ...
    private HourlyStatRequest mRequest;
    private HourlyStatResponse mData;
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String name = mData.getStats().get(i).getHours();
            if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(name, 100f*mData.getStats().get(i).getAvgVisits()/mData.getTotals().getAvgVisits());
    ...
    protected SourceLine.LineData getLineData(int position){
        final HoursStat data = mData.getStats().get(position);
        final HoursStat total = mData.getTotals();
        final HoursStat max = mData.getMax();
        String title = data.getHours()+":";
        String value = String.valueOf(data.getAvgVisits());
        ...
        for(int i=0;i< DataType.values().length;i++){
            if(DataType.values()[i]== DataType.VISITS){
                sublineData = new SourceSubline.SublineData(getString(R.string.line_visits), String.valueOf(data.getAvgVisits()), 100f*data.getAvgVisits()/total.getAvgVisits());
    ...
    protected void load(){
        ...
        mRequest = Webservice.getHourlyStat(!needRefresh(), new BaseRequest.Listener<HourlyStatResponse>() {
            @Override
            public void onResponse(HourlyStatResponse response) {
