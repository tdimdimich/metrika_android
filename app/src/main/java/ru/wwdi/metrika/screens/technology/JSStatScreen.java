 ...
 */
public class JSStatScreen extends BaseStatisticsScreen {
    ...
    private JSStatRequest mRequest;
    private JSStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final JSStat data = mData.getStats().get(position);
        final JSStat total = mData.getTotals();
        final JSStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getJSStat(!needRefresh(), new BaseRequest.Listener<JSStatResponse>() {
            @Override
            public void onResponse(JSStatResponse response) {
