 ...
 */
public class FlashStatScreen extends BaseStatisticsScreen {
    ...
    private FlashStatRequest mRequest;
    private FlashStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final FlashStat data = mData.getStats().get(position);
        final FlashStat total = mData.getTotals();
        final FlashStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getFlashStat(!needRefresh(), new BaseRequest.Listener<FlashStatResponse>() {
            @Override
            public void onResponse(FlashStatResponse response) {
