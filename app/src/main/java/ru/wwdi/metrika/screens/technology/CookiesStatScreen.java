 ...
 */
public class CookiesStatScreen extends BaseStatisticsScreen {
    ...
    private CookiesStatRequest mRequest;
    private CookiesStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final CookiesStat data = mData.getStats().get(position);
        final CookiesStat total = mData.getTotals();
        final CookiesStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getCookiesStat(!needRefresh(), new BaseRequest.Listener<CookiesStatResponse>() {
            @Override
            public void onResponse(CookiesStatResponse response) {
