 ...
 */
public class BrowsersStatScreen extends BaseStatisticsScreen {
    ...
    private BrowsersStatRequest mRequest;
    private BrowsersStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final BrowserStat data = mData.getStats().get(position);
        final BrowserStat total = mData.getTotals();
        final BrowserStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getBrowsersStat(!needRefresh(), new BaseRequest.Listener<BrowsersStatResponse>() {
            @Override
