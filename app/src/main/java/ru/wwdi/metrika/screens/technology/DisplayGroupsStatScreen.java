 ...
 */
public class DisplayGroupsStatScreen extends BaseStatisticsScreen {
    ...
    private DisplaysStatRequest mRequest;
    private DisplaysStatResponse mData;
    ...
    protected int getLinesCount(){
        ...
        return mData.getGroupStats().size();
    }
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String name = mData.getGroupStats().get(i).getName();
            if(mChartDataType== DataType.VIEWS){
                data = new Chart.ChartData(name, 100f*mData.getGroupStats().get(i).getPageViews()/mData.getTotals().getPageViews());
            }else if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(name, 100f*mData.getGroupStats().get(i).getVisits()/mData.getTotals().getVisits());
            }else if(mChartDataType== DataType.DENIAL){
                data = new Chart.ChartData(name, 100f*mData.getGroupStats().get(i).getDenial()/mData.getMax().getDenial());
            }else if(mChartDataType== DataType.VIEW_DEPTH){
                data = new Chart.ChartData(name, 100f*mData.getGroupStats().get(i).getDepth()/mData.getMax().getDepth());
            }else if(mChartDataType== DataType.VISIT_TIME){
                data = new Chart.ChartData(name, 100f*mData.getGroupStats().get(i).getVisitTime()/mData.getMax().getVisitTime());
            }
            result.add(data);
        }
        return result;
    ...
    protected SourceLine.LineData getLineData(int position){
        final DisplayGroupStat data = mData.getGroupStats().get(position);
        final DisplayGroupStat total = mData.getTotals();
        final DisplayGroupStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getDisplaysStat(!needRefresh(), new BaseRequest.Listener<DisplaysStatResponse>() {
            @Override
            public void onResponse(DisplaysStatResponse response) {
