 ...
 */
public class SearchPhrasesStatScreen extends BaseStatisticsScreen {
    ...
    private SearchPhrasesStatRequest mRequest;
    private SearchPhrasesStatResponse mData;
    ...
    protected int getLinesCount(){
        return mData.getSearchPhraseStats().size();
    }
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String phrase = mData.getSearchPhraseStats().get(i).getPhrase();
            if(mChartDataType== DataType.VIEWS){
                data = new Chart.ChartData(phrase, 100f*mData.getSearchPhraseStats().get(i).getPageViews()/mData.getTotals().getPageViews());
            }else if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(phrase, 100f*mData.getSearchPhraseStats().get(i).getVisits()/mData.getTotals().getVisits());
            }else if(mChartDataType== DataType.DENIAL){
                data = new Chart.ChartData(phrase, 100f*mData.getSearchPhraseStats().get(i).getDenial()/mData.getMax().getDenial());
            }else if(mChartDataType== DataType.VIEW_DEPTH){
                data = new Chart.ChartData(phrase, 100f*mData.getSearchPhraseStats().get(i).getDepth()/mData.getMax().getDepth());
            }else if(mChartDataType== DataType.VISIT_TIME){
                data = new Chart.ChartData(phrase, 100f*mData.getSearchPhraseStats().get(i).getVisitTime()/mData.getMax().getVisitTime());
            }
            result.add(data);
        }
        return result;
    ...
    protected SourceLine.LineData getLineData(int position){
        final SearchPhraseStat data = mData.getSearchPhraseStats().get(position);
        final SearchPhraseStat total = mData.getTotals();
        final SearchPhraseStat max = mData.getMax();
        String title = data.getPhrase()+":";
    ...
    protected void load(){
        ...
        mRequest = Webservice.getSearchPhrasesStat(!needRefresh(), new BaseRequest.Listener<SearchPhrasesStatResponse>() {
            @Override
            public void onResponse(SearchPhrasesStatResponse response) {
