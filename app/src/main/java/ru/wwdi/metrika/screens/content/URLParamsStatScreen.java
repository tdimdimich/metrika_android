 ...
 */
public class URLParamsStatScreen extends BaseStatisticsScreen {

    protected static enum DataType {
        VIEWS
    }

    private URLParamsStatRequest mRequest;
    private URLParamsStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final URLParamsStat data = mData.getStats().get(position);
        final URLParamsStat total = mData.getTotals();
        final URLParamsStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getURLParamsStat(!needRefresh(), new BaseRequest.Listener<URLParamsStatResponse>() {
            @Override
            public void onResponse(URLParamsStatResponse response) {
