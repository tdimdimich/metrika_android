 ...
 */
public class ExitPageStatScreen extends BaseStatisticsScreen {
    ...
    private ExitPageStatRequest mRequest;
    private ExitPageStatResponse mData;
    ...
    protected SourceLine.LineData getLineData(int position){
        final ExitPageStat data = mData.getStats().get(position);
        final ExitPageStat total = mData.getTotals();
        final ExitPageStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getExitPageStat(!needRefresh(), new BaseRequest.Listener<ExitPageStatResponse>() {
            @Override
            public void onResponse(ExitPageStatResponse response) {
