 ...
 */
public class GeoStatScreen extends BaseStatisticsScreen {
    ...
    private GeoStatRequest mRequest;
    private GeoStatResponse mData;
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String name = mData.getGeoStats().get(i).getName();
            if(mChartDataType== DataType.VIEWS){
                data = new Chart.ChartData(name, 100f*mData.getGeoStats().get(i).getPageViews()/mData.getTotals().getPageViews());
            }else if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(name, 100f*mData.getGeoStats().get(i).getVisits()/mData.getTotals().getVisits());
            }else if(mChartDataType== DataType.DENIAL){
                data = new Chart.ChartData(name, 100f*mData.getGeoStats().get(i).getDenial()/mData.getMax().getDenial());
            }else if(mChartDataType== DataType.VIEW_DEPTH){
                data = new Chart.ChartData(name, 100f*mData.getGeoStats().get(i).getDepth()/mData.getMax().getDepth());
            }else if(mChartDataType== DataType.VISIT_TIME){
                data = new Chart.ChartData(name, 100f*mData.getGeoStats().get(i).getVisitTime()/mData.getMax().getVisitTime());
            }
            result.add(data);
        }
        return result;
    ...
    protected SourceLine.LineData getLineData(int position){
        final GeoStat data = mData.getGeoStats().get(position);
        final GeoStat total = mData.getTotals();
        final GeoStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getGeoStat(!needRefresh(), new BaseRequest.Listener<GeoStatResponse>() {
            @Override
