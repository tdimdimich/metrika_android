 ...
 */
public class SearchEnginesStatScreen extends BaseStatisticsScreen {
    ...
    private SearchEnginesStatRequest mRequest;
    private SearchEnginesStatResponse mData;
    ...
    protected int getLinesCount(){
        return mData.getSearchEngineStats().size();
    }
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String name = mData.getSearchEngineStats().get(i).getName();
            if(mChartDataType== DataType.VIEWS){
                data = new Chart.ChartData(name, 100f*mData.getSearchEngineStats().get(i).getPageViews()/mData.getTotals().getPageViews());
            }else if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(name, 100f*mData.getSearchEngineStats().get(i).getVisits()/mData.getTotals().getVisits());
            }else if(mChartDataType== DataType.DENIAL){
                data = new Chart.ChartData(name, 100f*mData.getSearchEngineStats().get(i).getDenial()/mData.getMax().getDenial());
            }else if(mChartDataType== DataType.VIEW_DEPTH){
                data = new Chart.ChartData(name, 100f*mData.getSearchEngineStats().get(i).getDepth()/mData.getMax().getDepth());
            }else if(mChartDataType== DataType.VISIT_TIME){
                data = new Chart.ChartData(name, 100f*mData.getSearchEngineStats().get(i).getVisitTime()/mData.getMax().getVisitTime());
            }
            result.add(data);
        }
        return result;
    ...
    protected SourceLine.LineData getLineData(int position){
        final SearchEngineStat data = mData.getSearchEngineStats().get(position);
        final SearchEngineStat total = mData.getTotals();
        final SearchEngineStat max = mData.getMax();
    ...
    protected void load(){
        ...
        mRequest = Webservice.getSearchEnginesStat(!needRefresh(), new BaseRequest.Listener<SearchEnginesStatResponse>() {
            @Override
            public void onResponse(SearchEnginesStatResponse response) {
