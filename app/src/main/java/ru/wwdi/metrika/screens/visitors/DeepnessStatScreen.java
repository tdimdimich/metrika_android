 ...
 */
public class DeepnessStatScreen extends BaseStatisticsScreen {
    ...
    protected int getLinesCount(){
        ...
        return mData.getDeepnessStats().size();
    }
    ...
    protected List<Chart.ChartData> getChartData(){
        ...
        for(int i=0;i<getLinesCount();i++){
            String name = mData.getDeepnessStats().get(i).getName();
            if(mChartDataType== DataType.VISITS){
                data = new Chart.ChartData(name, 100f*mData.getDeepnessStats().get(i).getVisitsPercent());
            }else if(mChartDataType== DataType.DENIAL){
                data = new Chart.ChartData(name, 100f*mData.getDeepnessStats().get(i).getDenial()/mData.getMax().getDenial());
            }else if(mChartDataType== DataType.VIEW_DEPTH){
                data = new Chart.ChartData(name, 100f*mData.getDeepnessStats().get(i).getDepth()/mData.getMax().getDepth());
            }else if(mChartDataType== DataType.VISIT_TIME){
                data = new Chart.ChartData(name, 100f*mData.getDeepnessStats().get(i).getVisitTime()/mData.getMax().getVisitTime());
            }
            result.add(data);
        }
        return result;
    ...
    protected SourceLine.LineData getLineData(int position){
        final DeepnessStat data = mData.getDeepnessStats().get(position);
